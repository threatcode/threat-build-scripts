...
[ -z $user ] && user=pi
[ -z $password ] && password=threat
[ -z $desktop ] && desktop=no
[ -z $hostname ] && hostname=threat
...
echo -e " ${whiteColor}Threat Edition: ${cyanColor}$edition"
...
debootstrap --arch=$architecture --components=main,contrib,non-free --include=gnupg2,nano,base-files threat $edition-$architecture https://deb.threat.sh/direct/threat/
...
chroot $edition-$architecture bash -c "apt -y install threat-core-lite"
chroot $edition-$architecture bash -c "apt update"
...
if [ $edition == "home" ] || [ $edition == "security" ]; then
	chroot $edition-$architecture bash -c "apt -y install threat-interface threat-desktop-mate firefox-esr abiword chromium- mate-user-guide- pocketsphinx-en-us- libreoffice-help-en-us- mythes-en-us- libreoffice-help-common- espeak-ng-data-"
	chroot $edition-$architecture bash -c "apt -y install anonsurf anonsurf-gtk" || true
fi
if [ $edition == "security" ]; then
	chroot $edition-$architecture bash -c "apt -y install -t threat-backports threat-tools-automotive threat-tools-cloud threat-tools-infogathering threat-tools-maintain threat-tools-password threat-tools-postexploit threat-tools-pwn threat-tools-sniff threat-tools-vuln threat-tools-web threat-tools-wireless mate-user-guide- pocketsphinx-en-us- libreoffice-help-en-us- mythes-en-us- libreoffice-help-common- espeak-ng-data- android-sdk- arduino- chromium- chromium-driver- dradis- exploitdb- ferret- fonts-noto-core- freepats- gcc-avr- gnuradio*- guile-2.2-libs- gvim- imagemagick*- llvm-11-dev- lmodern- minicom- pandoc*- plasma-discover- plasma-discover-common- python-all- python-all-dev- python-mpltoolkits.basemap-data- qttools5-dev-tools- seclists- tex-*- tex-gyre- thunderbird- vim- wine- wine64- xpdf- xpra- termit-"
fi
...
chroot $edition-$architecture bash -c "/usr/share/threat-menu/update-launchers" || true
...
tar czvf images/Threat-$edition-$device-${version}_${architecture}.tar.gz -C $edition-$architecture .
...
if [ $verbose = yes ]; then
	vmdb2 --rootfs-tarball=images/Threat-$edition-$device-${version}_${architecture}.tar.gz --output images/Threat-$edition-$device-${version}_${architecture}.img work_dir/recipe.yaml --verbose --log work_dir/build.log
else
	vmdb2 --rootfs-tarball=images/Threat-$edition-$device-${version}_${architecture}.tar.gz --output images/Threat-$edition-$device-${version}_${architecture}.img work_dir/recipe.yaml --log work_dir/build.log
fi
...
xz --best --extreme images/Threat-$edition-$device-${version}_${architecture}.img

# # Compress and finalize image
# echo -e "$dot$greenColor Compressing and finalizing image...$resetColor"

# PARTNAME=$(kpartx -f images/Threat-$edition-$device-${version}_$architecture-orig.img | cut -d ' ' -f 1 | grep p2 | sed -e 's/p2//')
# TEMP=$(mktemp -d)
# kpartx -av images/Threat-$edition-$device-${version}_$architecture-orig.img
# mount /dev/mapper/${PARTNAME}p2 $TEMP
# USED=$(df --output=used "$TEMP" | sed '1d;s/[^0-9]//g')
# umount $TEMP
# rm -r $TEMP
# kpartx -d images/Threat-$edition-$device-${version}_$architecture-orig.img
# NEWSIZE=$(echo "$USED/1024+100+260" | bc)
# NEWDATASIZE=$(echo "$USED/1024+94" | bc)

# qemu-img create -f raw images/compr.img ${NEWSIZE}M
# sfdisk --quiet --dump images/Threat-$edition-$device-${version}_$architecture-orig.img | grep -v img2 | sfdisk --quiet --force images/compr.img
...
# w
# EOF
# readarray rmappings < <(sudo kpartx -asv images/Threat-$edition-$device-${version}_$architecture-orig.img)
...
# kpartx -ds images/Threat-$edition-$device-${version}_$architecture-orig.img
# kpartx -ds images/compr.img
# rm images/Threat-$edition-$device-${version}_$architecture-orig.img
# mv images/compr.img images/Threat-$edition-$device-${version}_$architecture.img
