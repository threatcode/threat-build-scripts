...
installer_image_name() {
	if [ "$THREAT_VARIANT" = "netinst" ]; then
		echo "architect/images/threat-$THREAT_VERSION-$THREAT_ARCH-NETINST-1.iso"
	else
		echo "architect/images/threat-$THREAT_VERSION-$THREAT_ARCH-DVD-1.iso"
	fi
}
...
target_image_name() {
	...
	if [ "$IMAGE_TYPE" = "live" ]; then
		if [ "$THREAT_VARIANT" = "default" ]; then
			echo "${TARGET_SUBDIR:+$TARGET_SUBDIR/}Threat-home-${THREAT_VERSION}_$THREAT_ARCH.$IMAGE_EXT"
		else
			echo "${TARGET_SUBDIR:+$TARGET_SUBDIR/}Threat-$THREAT_VARIANT-${THREAT_VERSION}_$THREAT_ARCH.$IMAGE_EXT"
		fi
	else
		if [ "$THREAT_VARIANT" = "default" ]; then
			echo "${TARGET_SUBDIR:+$TARGET_SUBDIR/}Threat-architect-DVD-${THREAT_VERSION}_$THREAT_ARCH.$IMAGE_EXT"
	    elif [ "$THREAT_VARIANT" = "netinst" ]; then
			echo "${TARGET_SUBDIR:+$TARGET_SUBDIR/}Threat-architect-${THREAT_VERSION}_$THREAT_ARCH.$IMAGE_EXT"
		else
			echo "${TARGET_SUBDIR:+$TARGET_SUBDIR/}Threat-architect-$THREAT_VARIANT-${THREAT_VERSION}_$THREAT_ARCH.$IMAGE_EXT"
		fi
	fi
}
...
THREAT_CONFIG_OPTS="--distribution $THREAT_DIST -- --variant $THREAT_VARIANT --version $THREAT_VERSION"
CODENAME=$THREAT_DIST # for architect/debian-cd
if [ -n "$OPT_pu" ]; then
...
case "$IMAGE_TYPE" in
	live)
		if [ ! -d "$(dirname $0)/templates/variant-$THREAT_VARIANT" ]; then
	...
	;;
	installer)
		if [ ! -d "$(dirname $0)/templates/installer-$THREAT_VARIANT" ]; then
...
case "$IMAGE_TYPE" in
	iot)
		HERE=$(pwd)
		cd iot
		./build.sh --version $THREAT_VERSION --edition $THREAT_VARIANT --arch $THREAT_ARCH --verbose yes
		cd $HERE
	;;
	rootfs)
		HERE=$(pwd)
		cd rootfs
		./build.sh $THREAT_VERSION
		cd $HERE
	;;
	live)
	...
	installer)
		...
		if [ -e .mirror ]; then
			...
			threat_mirror=https://deb.threat.sh/direct/threat
		fi
		...
		grep -v '^#' templates/installer-$THREAT_VARIANT/packages \
			> architect/profiles/threat.downloads
		[ $? -eq 0 ] || failure

		# Tasksel is required in the mirror for debian-cd
		echo tasksel >> architect/profiles/threat.downloads
		[ $? -eq 0 ] || failure
		...
		if [ "$THREAT_ARCH" = "arm64" ]; then
			debug "arm64 GRUB"
			echo "grub-efi-arm64" >> architect/profiles/threat.downloads
			[ $? -eq 0 ] || failure
		fi
